package top.yzlin.cqrobotsdk.keywordfunc;
...
public class KeyWord {
    ...
    public KeyWord(String groupID,CQRobot cqRobot, Collection<String> admin){
        this(groupID,cqRobot);
        adminSet.addAll(admin);
    }

    public KeyWord(String groupID,CQRobot cqRobot){
        ...
        //创建xml文档或载入文档
        xmlFile=new File("doc\\"+Tools.MD5(groupID).substring(5,15)+".xml");
        if(!xmlFile.exists()){
            try {
    ...
    public boolean addAdmin(String QQID){
        Tools.print("为"+ groupID +"添加了新的管理员"+QQID);
    ...
    public boolean removeAdmin(String QQID){
        Tools.print("为"+ groupID +"删除了管理员"+QQID);
